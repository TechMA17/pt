...
import com.citi.posttradeanalyzer.model.VampPriceTimePairView;
import com.citi.posttradeanalyzer.model.VwampView;
...
public class QueryEngine {
	...
	public HashMap<String, String> getNumberOfIOCFilledOrder() {
		...
		query.select(cb.tuple(cb.count(root), root.get("transactTime")));
		Predicate p1 = cb.equal(root.get("timeInForce"), "3");
	...
	public ArrayList<ArrayList<String>> getSamplePricePerMinute(String symbol) {
		System.out.println("inside getsamplepriceperminute");
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<Tuple> query = cb.createTupleQuery();
		Root<VampPriceTimePairView> root = query.from(VampPriceTimePairView.class);
		query.select(cb.tuple(root.get("totalPrice"), root.get("startTime")));
	...
	public ArrayList<ArrayList<String>> getVwampValueEveryFiveMins(String symbol) {
		System.out.println("inside getVwampValueEveryFiveMins");
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<Tuple> query = cb.createTupleQuery();
		Root<VwampView> root = query.from(VwampView.class);
		query.select(cb.tuple(root.get("totalQty"), root.get("sumProduct"), root.get("startTime")));
		Predicate p1 = cb.equal(root.get("symbol"), symbol);
		query.where(cb.and(p1));
		...
		for (Tuple result : results) {
			innerArr = new ArrayList<>();
			long totalQty = (long) result.get(0);
			long sumProduct = (long) result.get(1);
			double num = sumProduct / totalQty;
			System.out.println(totalQty + " " + sumProduct + " " +  num);
		
			innerArr.add(String.valueOf(result.get(2)));
			innerArr.add(String.valueOf(num));
			arr.add(innerArr);
		}
		return arr;
	}
